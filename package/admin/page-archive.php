<!-- ======= Start package/admin/PAGE-ARCHIVE.PHP ======= -->
    ...
    <body onload="myFunction()">
        ...
        <div class="main-wrapper">
                ...
                <div class="content container-fluid">
                    ...
                    <div class="alert alert-warning">
						<i class="fa fa-info-circle"></i>&nbsp; 
                            <strong>GUIDE |</strong> &nbsp; Manage Admin Archive files. ( Search / Add / Edit / Download / Delete ).
				    </div>
					...
					<div class="row">
						...
						if(isset($_POST['searchBtn'])
							...
							$query = mysqli_query($conn, "SELECT * FROM `tbl_admindocuments` WHERE file_uploaded BETWEEN '$newFromDate' AND '$newToDate'") or die(mysqli_error($conn));

						} else {
							$query = mysqli_query($conn, "SELECT * FROM `tbl_admindocuments`") or die(mysqli_error($conn));
						}
			...
			<?php include('config.php'); ?>
				<div class="row">
					<div class="col-md-12">
						<form method="POST" action="add-archive.php" enctype=multipart/form-data>
							<div class="row">
								<div class="col-md-4">
									<div class="form-group">
										<input type="file" name="adoc_file" class="form-control"/>
									</div>
									<div class="form-group">
										<input type="text" name="adoc_title" placeholder="Title" class="form-control"/>
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
										<textarea type="text" name="adoc_description" cols="8" rows="4" placeholder="Description" class="form-control"></textarea>
									</div>
								</div>

								<div class="col-md-4">
									<div class="form-group">
										<input type="text" name="schoolyear" value="<?php echo $curr = date('Y')+1; date('Y')."-".$curr; ?>" class="form-control" readonly/>
									</div>
									<div class="form-group">
		...
		<div id = "content">
			<table id = "table" class="table table-bordered">
				...
				<tbody>
					<?php while($fetch = mysqli_fetch_array($query) ){?>
						...
						<tr class="del_adoc<?php echo $fetch['adoc_id'] ?>">
							<td class="text-center">
								<?php
									$array = explode(".",$fetch['adoc_path']);
									$extension = $array[count($array)-1];
								?>
								<a title="Click this to download" href="<?php echo $fetch['adoc_path'] ?>" download="<?php echo $fetch['adoc_title'] . ".".$extension ?>">
									<?php echo $fetch['adoc_title'] ?>
								</a>
							</td>
							<td class="text-center"><?php echo $fetch['adoc_description'] ?></td>
							<td class="text-center"><?php echo $fetch['adoc_path'] ?></td>
							<td class="text-center"><?php echo $fetch['adoc_schoolyear'] ?></td>
							...
							<td class="text-center">	
								<div class="dropdown dropdown-action">
									<a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item" data-toggle="modal" data-target="#edit_modal<?php echo $fetch['adoc_id']?>"><i class="fa fa-pencil m-r-5"></i> Edit</a>
											<a class="dropdown-item btn-delete" id="<?php echo $fetch['adoc_id']?>" type="button"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
										</div>
								</div>
							</td>
						</tr>
			...
			<!-- Update File -->
			<div class="modal fade" id="edit_modal<?php echo $fetch['adoc_id']?>" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content">
						<form method="POST" action="update-archive.php">	
							...
							<div class="modal-body">
								<div class="col-md-12">
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												<input type="hidden" name="adoc_id" value="<?php echo $fetch['adoc_id']?>" class="form-control"/>
												<label>Title</label>
												<input type="text" name="adoc_title" value="<?php echo $fetch['adoc_title']?>" class="form-control"/>
											</div>
											<div class="form-group">
												<label>Description</label>
												<input type="text" name="adoc_description" value="<?php echo $fetch['adoc_description']?>" class="form-control"/>
											</div>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('.btn-delete').on('click', function(){
					var adoc_id = $(this).attr('id');
					$("#modal_confirm").modal('show');
					$('#btn_yes').attr('name', adoc_id);
				});
				$('#btn_yes').on('click', function(){
					var id = $(this).attr('name');
					$.ajax({
						type: "POST",
						url: "delete-archive.php",
						data:{
							adoc_id: id
						},
						success: function(){
							...
							$(".del_adoc" + id).empty();
							$(".del_adoc" + id).html("<td colspan='6'><center class='text-danger'>Deleting...</center></td>");
							setTimeout(function(){
								$(".del_adoc" + id).fadeOut('slow');
							}, 1000);
						}
					});
				});
...
<!-- ======= End package/admin/PAGE-ARCHIVE.PHP ======= -->
